...
# 1. CONFIGURACIÓN
st.set_page_config(page_title="Inversa - Pase de Alcohol DUSA", page_icon="🧪", layout="centered")

# Mantengo tu CSS original que tanto te gustó
st.markdown("""
    <style>
...
def obtener_fp(grado):
    # Aquí pegas tu matriz completa de 501 valores que ya editaste manualmente
    puntos = {
...
st.markdown(f"""
    <div class="header-container">
        ...
        <div>
            <p class="titulo-mini">Calculadora Inversa 2.0</p>
            <p class="subtitulo-mini">Determinación de LAA por Peso</p>
            <p class="autor-text">© Edwin Freitez</p>
        </div>
    </div>
    """, unsafe_allow_html=True)

# ENTRADA DE DATOS INVERTIDA
c1, c2 = st.columns(2)
with c1:
    entrada_g = st.number_input("Grado Real (°GL):", 50.0, 100.0, 96.0, 0.1, format="%.1f")
with c2:
    # Ahora el dato de entrada es el Peso de la Romana
    peso_bruto = st.number_input("Peso en Romana (Kg):", min_value=0.0, value=None, step=0.1, placeholder="Ingrese Kg")
...
if st.button("CALCULAR LAA", use_container_width=True):
    if peso_bruto is not None:
        ...
        if fp:
            # --- FÓRMULAS INVERTIDAS ---
            vol_bruto = peso_bruto * fp
            laa = (vol_bruto * entrada_g) / 100
            
            # Formateo
            laa_fmt = "{:,}".format(round(laa, 2)).replace(',', 'X').replace('.', ',').replace('X', '.')
            v_fmt = "{:,}".format(round(vol_bruto, 1)).replace(',', 'X').replace('.', ',').replace('X', '.')
            
            st.write(f"Resultados para **{peso_bruto} Kg** a **{entrada_g}°GL**:")
            
            # RESULTADOS (Invertidos)
            st.metric(label="🧪 LAA CALCULADOS:", value=f"{laa_fmt} Lts")
        ...
        st.warning("Por favor, ingrese el peso registrado en romana.")
